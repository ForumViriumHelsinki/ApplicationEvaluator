...
from . import print_evaluation_table
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--close-challenge", action="store_true", help="Close challenge evaluation round")
        ...
        with transaction.atomic():
            if options["move_to_tech_evaluation"]:
                if options["tech_email"]:
                    ...
                    exit(1)
            elif options["close_challenge"]:
                log = ["Close challenge evaluation round"]
                ar.published = False
                ar.save()
                log.append(f"Closed: {ar.name}")
            if options["dry_run"]:
