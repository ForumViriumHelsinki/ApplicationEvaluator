...
> {
  ...
  render() {
    ...
    const myOrgScore = application.scores.find(
      (s) => s.criterion === criterion.id && s.evaluator.organization === user.organization,
    );
    ...
    return (
      <div className="ml-2 mb-2">
        ...
        {scores.map((score: Score) => (
          <div key={score.id}>
            <strong>{showScores || score.evaluator.id === user.id ? score.score : '?'}</strong> (by{' '}
            {username(score.evaluator)})
