...
export default class ApplicationScores extends React.Component<
	ApplicationScoresProps,
	ApplicationScoresState
> {
	state = initialState;
	...
	render() {
		...
		const thresholdGroups = applicationRound.criterion_groups.filter(
			(g) => g.threshold,
		);
		...
		return (
			<div className={`mt-4 pb-4 app-${application.id}`}>
				<div className="d-flex mr-4">
					{highlightOrganization && (
						<style>
							.app-{application.id} .org-shape{" "}
							{"{opacity: 0.2; transition: opacity 100ms;}"}
							.app-{application.id} .org-shape-{slug(highlightOrganization)}{" "}
							{"{opacity: 1}"}
						</style>
					)}
					{thresholdGroups.length > 1 && (
						<div
							style={{ width: 200, marginBottom: -48, marginTop: -38 }}
					...
					<div
						className={`flex-grow-1 flex-shrink-1 ${thresholdGroups.length < 2 ? "ml-4" : ""}`}
					>
						<a
							...
							<div className="mb-1">
								{application.evaluating_organizations.map((o) => (
									<span
										...
										style={{ color: this.organizationColor(o) }}
										key={o}
									>
										{o}
									</span>
						...
						{application.score != null ? (
							showEvaluators ? (
								<>
									...
									overall from {application.scores.length} scores for{" "}
									{scoredCriteria.length} criteria.
									<br />
									Evaluated by{" "}
									{_.uniq(
										application.scores.map((s) => username(s.evaluator)),
									).join(", ")}
									.
								</>
							) : (
				...
				{application.score != null && (
					<div className="pl-4 pr-4">
						...
						{showPlot && (
							<small>
								Move mouse over table to highlight organization in plot.
							</small>
						)}
					</div>
				)}
				...
				{expanded && (
					...
					>
						...
						<h3 className="mt-3 p-2 bg-primary text-white">
							Application evaluation
						</h3>
						{application.scores.length > 0 && (
							<div className="m-2">
								<ExportScoresWidget applicationRound={applicationAsRound} />
							</div>
						)}
	...
	plotData() {
		...
		const data = (groupScores: any) =>
			Object.fromEntries(
				thresholdGroups.map((g) => [
					g.id,
					(groupScores[g.id] || 1) / settings.maxScore,
				]),
			);

		const scoresIndex =
			applicationRound.scoring_model === "Evaluators average"
				? application.scoresByEvaluator
		...
		return Object.entries(scoresIndex).map(([org, { groupScores }]) => ({
			data: data(groupScores),
			meta: {
				color: organizationColor(org),
				class: `org-shape org-shape-${slug(org)}`,
			},
		}));
	}
	...
	plotOptions() {
		...
		const thresholds = Object.fromEntries(
			thresholdGroups.map((g) => [g.id, g.threshold]),
		);
		...
		return {
			...
			captionProps: ({ key }: any) => {
				...
				const textAnchor =
					groupIndex === 0 || groupIndex === middle
						? "middle"
						: groupIndex < middle
							? "start"
							: "end";
				...
				return score == null
					...
					: score < thresholds[key]
						? {
								fontWeight: "bold",
								textAnchor,
								class: "chart-caption fill-danger",
							}
	...
	highlightOrganization = (highlightOrganization: string | null) => {
		...
		this.endHighlight = setTimeout(
			() => this.setState({ highlightOrganization: null }),
			1000,
		);
	};
