...
import { getTotalScore, organizationColor, slug, username } from '../components/utils';
...
> {
            ...
            {application.score != null ? (
              showEvaluators ? (
                <>
                  ...
                  overall from {application.scores.length} scores for {scoredCriteria.length}{' '}
                  criteria.
                  <br />
                  Evaluated by{' '}
                  {_.uniq(application.scores.map((s) => username(s.evaluator))).join(', ')}.
                </>
              ) : (
                <>
                  <strong>
  ...
  plotData() {
    ...
    const data = (groupScores: any) =>
      Object.fromEntries(
        thresholdGroups.map((g) => [g.id, (groupScores[g.id] || 1) / settings.maxScore]),
      );
