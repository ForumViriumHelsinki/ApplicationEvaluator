...
import type { Application, ApplicationRound, CriterionGroup } from '/components/types';
  ...
  render() {
    ...
    const GroupScore = ({ group, groupScores }: { group: CriterionGroup; groupScores: any }) => {
      ...
      return (
        <td
          key={group.id}
          className={groupScore < group.threshold ? 'text-danger font-weight-bold' : ''}
        >
    ...
    const LegendPill = ({ org }: { org: string }) => (
      <span className="rounded-pill d-inline-block" style={this.legendPillStyle(org)} />
    );
    ...
    return (
      <table className="table table-hover table-sm mt-2 border-bottom mb-0">
        ...
        <tbody onMouseOut={selectNone}>
          {showOrganizations &&
            Object.entries(scoresIndex).map(([organization, { groupScores, score }]) => (
              <tr key={organization} onMouseOver={() => onOrganizationHover?.(organization)}>
                <td>
