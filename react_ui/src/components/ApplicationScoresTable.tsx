...
export default class ApplicationScoresTable extends React.Component<
	ApplicationScoresTableProps,
	ApplicationScoresTableState
> {
	state = initialState;
	...
	render() {
		...
		const GroupScore = ({
			group,
			groupScores,
		}: { group: CriterionGroup; groupScores: any }) => {
			const groupScore = groupScores[group.id];
			return (
				<td
					key={group.id}
					className={
						groupScore < group.threshold ? "text-danger font-weight-bold" : ""
					}
				>
					{groupScore?.toPrecision(2)}
				</td>
			);
		};
		...
		const LegendPill = ({ org }: { org: string }) => (
			<span
				...
				style={this.legendPillStyle(org)}
			/>
		);

		const selectNone = () =>
			onOrganizationHover && showOrganizations && onOrganizationHover(null);

		return (
			<table className="table table-hover table-sm mt-2 border-bottom mb-0">
				<thead>
					<tr>
						{showOrganizations && (
							<>
						...
						{thresholdGroups.map((group) => (
							<th key={group.abbr}>{group.abbr}</th>
						))}
					</tr>
				</thead>
				<tbody onMouseOut={selectNone}>
					{showOrganizations &&
						Object.entries(scoresIndex).map(
							([organization, { groupScores, score }]) => (
								<tr
									key={organization}
									onMouseOver={() => onOrganizationHover?.(organization)}
								>
									<td>
										<LegendPill org={organization} /> {organization}
									</td>
						...
						{showOrganizations && (
							<>
								...
								<td className="font-weight-bold">
									{application.score?.toPrecision(3)}
								</td>
							</>
						)}
						{thresholdGroups.map((group) => (
							<GroupScore
								key={group.id}
								group={group}
								groupScores={application.groupScores}
							/>
						))}
					</tr>
				</tbody>
