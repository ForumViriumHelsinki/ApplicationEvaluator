...
function childGroups(
	...
	return applicationRound.criterion_groups.filter(
		(g) => g.parent === criterionGroup.id,
	);
}
...
const allCriteria = (
	...
	return _.flatten([
		...
		// @ts-ignore
		childGroups(applicationRound, criterionGroup).map((g) =>
			allCriteria(applicationRound, g),
		),
	]);
};

export const addApplicationScores = (
	...
	const criteriaByGroup = Object.fromEntries(
		round.criterion_groups.map((g) => [g.id, allCriteria(round, g)]),
	);
	const weightIndex = Object.fromEntries(
		round.criteria.map((c) => [c.id, c.weight]),
	);

	const weightedAvg = (scores: Score[]) => {
		const scoresByCriterion = _.groupBy(scores, "criterion");
		const avgScores = Object.entries(scoresByCriterion).map(
			([criterion, cScores]) => {
				const orgScores = Object.values(
					_.groupBy(cScores, (s) => s.evaluator.organization),
				).map((scores) => avg(scores.map((s) => s.score)));
				return { criterion, score: avg(orgScores) };
			},
		);
		const weightedScores = avgScores.map(
			(s) => s.score * weightIndex[s.criterion],
		);
	...
	const groupScores = (scores: Score[]) => {
		...
		thresholdGroups.forEach((group) => {
			const criterionIndex = Object.fromEntries(
				criteriaByGroup[group.id].map((c) => [c.id, c]),
			);
	...
	applications.forEach((app) => {
		...
		app.scored =
			_.uniq(app.scores.map((s: Score) => s.criterion)).length ===
			round.criteria.length;
		app.score = weightedAvg(app.scores);
		app.groupScores = groupScores(app.scores);

		const scoresByOrganization = _.groupBy(
			app.scores,
			(s) => s.evaluator.organization,
		);
		app.scoresByOrganization = Object.fromEntries(
			Object.entries(scoresByOrganization).map(([organization, scores]) => [
				organization,
				{ score: weightedAvg(scores), groupScores: groupScores(scores) },
			]),
		);

		const scoresByEvaluator = _.groupBy(app.scores, (s) =>
			username(s.evaluator),
		);
		app.scoresByEvaluator = Object.fromEntries(
			Object.entries(scoresByEvaluator).map(([evaluator, scores]) => [
				evaluator,
				{ score: weightedAvg(scores), groupScores: groupScores(scores) },
			]),
		);
...
export const organizationColor = (org: string) => {
	if (!organizationColors[org])
		organizationColors[org] = colors[colorIndex++ % colors.length];
	return organizationColors[org];
};

export const slug = (name: string) => name.replace(/[^a-z]/gi, "");

export function getTotalScore(application: Application) {
	return (
		application.score &&
		(application.score * settings.finalScoreMultiplier).toPrecision(3)
	);
}
