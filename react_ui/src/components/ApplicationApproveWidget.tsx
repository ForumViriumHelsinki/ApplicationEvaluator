...
import type { AppContextType, Application, ApplicationRound } from '/components/types';
...
> {
  ...
  render() {
    ...
    const submitted =
      applicationRound.scoring_completed || applicationRound.submitted_organizations.length > 0;
    if (!(user.is_superuser && submitted)) return null;

    return application.approved ? (
      <button type="button" className={`${className} btn-success`} onClick={this.unapprove}>
        Approved
      </button>
    ) : (
      <button type="button" className={`${className} btn-outline-success`} onClick={this.approve}>
        Approve
      </button>
    );
  }
