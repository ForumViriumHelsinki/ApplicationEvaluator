...
import { AppContext, type AppContextType, type ApplicationRound } from '/components/types';
...
> {
  ...
  render() {
    ...
    const uniqueEvaluators = _.uniq(
      _.flatten(applicationRound.applications.map((a) => a.scores.map((s) => s.evaluator))),
    );
          ...
          {expanded &&
            ...
            partialApps.length === applicationRound.applications.length && (
              <ConfirmButton
                ...
                confirm={
                  <>
                    Submit all {applicationRound.name} scores for {user.organization}? Scores cannot
                    be changed after submitting.
                    <div className="mt-2">
                      {scoredApps.length}/{applicationRound.applications.length} applications
            ...
            <div className="mt-2">
              ...
              <div className="form-check d-inline-block mr-3">
                <input
                  ...
                  onChange={() => this.setState({ showEvaluators: !showEvaluators })}
                />
                ...
                applicationRound.scoring_model !== 'Evaluators average' && (
                  <div className="form-check d-inline-block mr-3">
                    <input
                      ...
                      id="showScores"
                      checked={showScores}
                      onChange={() => this.setState({ showScores: !showScores })}
                    />
  ...
  getApplications() {
    ...
    if (order === 'unevaluated') apps.sort((a, b) => a.scores.length - b.scores.length);
    else if (order === 'score') apps.sort((a, b) => (b.score || 0) - (a.score || 0));
    return apps;
  }
